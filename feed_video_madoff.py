...
def update_newest_video(videoEntry):
    ...
    try:
        cur.execute('SELECT * FROM `madoff_waiting_process` WHERE videoid = \'' + videoEntry['videoID'] + '\'')
        entry = cur.fetchone()

        if entry is None:
            sqlInsert = 'INSERT INTO `madoff_waiting_process` ' \
...
def feedMadoff_main():
    ...
    ##############################################################################################
    # YOUTUBER CHANNEL ID
    username = "UCIALMKvObZNtJ6AmdCLP7Lg"
    url = "https://www.youtube.com/channel/" + username + "/videos?view=0&sort=dd&shelf_id=0"

    print("Start Feeding Video link to Madoff DB")
    ...
    print("End of feed Madoff Process")
