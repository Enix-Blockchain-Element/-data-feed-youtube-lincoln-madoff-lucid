...
def update_newest_video(videoEntry):
    ...
    try:
        cur.execute('SELECT * FROM `lucid_waiting_process` WHERE videoid = \'' + videoEntry['videoID'] + '\'')
        entry = cur.fetchone()

        if entry is None:
            sqlInsert = 'INSERT INTO `lucid_waiting_process` ' \
...
def feedlucid_main():
    ...
    ##############################################################################################
    # YOUTUBER CHANNEL ID
    username = "UCytNzxSmUqEBychgoKoQssw"
    url = "https://www.youtube.com/channel/"+username+"/videos?view=0&sort=dd&shelf_id=0"

    print("Start Feeding Video link to Lucid DB")
    ...
    print("End of feed Lucid Process")
